...
class FaceRecognitionPipeline:
    ...
    def _alignment_worker(self):
        while self.running:
            try:
                data = self.align_queue.get(timeout=0.1)
                if data is None:
                    continue
    ...
    def _recognition_worker(self):
        while self.running:
            try:
                data = self.recog_queue.get(timeout=0.1)
                if data is None:
                    continue
                ...
                similarities = [F.cosine_similarity(torch.tensor(e).to(self.device), embedding).item() 
                              for e in self.saved_embeddings]
